...
\begin{equation} \label{eq:angALL}
  \begin{split}
    ...
    & + Y_{B}\,B^m(m) \, B^{\TK}(\TK) \, B^{\TL}(\TL) \, B^{\varphi}(\varphi), \\
  \end{split}
\end{equation}
...
%% The function $S^a(\TK,\TL,\varphi)$ describes the signal in the three-dimensional (3D) space of the angular variables and corresponds to Eq.~(\ref{eq:PDF}).

The four \pdfs $B^m(m) \, B^{\TK}(\TK) \, B^{\TL}(\TL) \, B^{\varphi}(\varphi)$ describe the background in the space of the \PBz candidate invariant mass and the angular variables.
The mass \pdf $B^m(m)$ is an exponential function, The angular \pdfs $B^{\TK}(\TK)$ and $B^{\TL}(\TL)$ are polynomials functions, ranging from second to forth degree depending on the $q^2$ bin, and $B^{\varphi}(\varphi)$ is a first-order polynomial function.
...
\subsubsection{Fraction of mis-tagged signal events}
\label{sec:mistag}
...
The algorithm used to tag the CP-state of the \PBz candidate, described in Section~\ref{sec:selection}, has an intrinsic percentage of failure which is referred to as mistag fraction, $f^M_i$, defined as the ratio of mistagged signal events divided by the total number of signal events.
...
Some eventual differences between simulated and real events could lead to a bias in the mistag fraction values used in the \pdf, that could propagate to a bias in the analysis results.
The contribution of this effect to the systematic uncertainty, is discussed in Section~\ref{sec:sys-mistag}.

\subsubsection{Background parameterisation}
\label{sec:backg}
...
Since the four-body invariant mass distribution of these events does not have any structure, their contribution can be evaluated from the mass sidebands, and extrapolated to the full mass range.

In addition, no correlation is expected between the four-body mass distribution and the angular variable distributions.
This allows to estimate the shapes of the angular distributions on the sidebands, and consider them valid also to describe the combinatorial background contamination in the \PBz mass region.

The \pdf used to model the angular shape of this background is the product of three uncorrelated polynomial functions, $B^{\TK}(\TK) \, B^{\TL}(\TL) \, B^{\varphi}(\varphi)$.
...
The polynomial degrees used are listed in the Table~\ref{tab:back-degree}.
...
\begin{table}[!htb]
  \begin{center}
    ...
    \begin{tabular}{c|c|c|c}
      $q^2$ bin & $B_i^{\cos\theta_\mathrm{K}}$ & $B_i^{\cos\theta_l}$ & $B_i^{\phi}$ \\
...
An example of the sideband event distributions, for $q^2$ bin~0, are plotted from Figure~\ref{fig:bin0-bkg-l} to Figure~\ref{fig:bin0-bkg-phi}, together with the projections of the combinatorial background \pdf on the angular variables, after the fit to the sideband distributions described in Section~\ref{sec:fitseq}.
...
The correlation of the variable distributions for the combinatorial background events is tested and the results shown in this section.
...
To take into account the effect of eventual difference between simulated and real events, some systematic uncertainties have been computed and will be described in Section~\ref{sec:syst}.
...
The dependence of the fit result as a function of the region width, was tested and is found to be negligible.
